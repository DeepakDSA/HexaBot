...
// Servo Motors
Servo servos[12];
const int servoPins[12] = {13, 14, 25, 26, 27, 32, 33, 15, 4, 5, 18, 19};
...
void sayAndDoNamaste() {
    ...
    myDFPlayer.play(28);  // Play "namaste" sound (028.mp3)
    delay(1000); // Wait for audio to start

    // Assuming front two leg servos are at index 0 and 1
    servos[0].write(60);  // Move servo 0 towards center
    servos[1].write(120); // Move servo 1 towards center
...
void playVoiceResponse(String command) {
    command.toLowerCase();  // Convert input to lowercase for case-insensitive match
    for (int i = 0; i < numCommands; i++) {
        if (command.indexOf(mp3Files[i].keyword) != -1) {  // Check if keyword exists in text
            Serial.println("Playing: " + mp3Files[i].filename);
            myDFPlayer.play(i + 1);  // Play the MP3 file (assuming ordered on SD card)
            return;
        }
    }
...
void danceMove() {
    ...
    delay(500);
    if (myDFPlayer.available()) myDFPlayer.play(2); // Play a dance song
}
...
void setup() {
    ...
    if (!setupDFPlayer()) {
        Serial.println("DFPlayer setup failed! Skipping.");
    }
...
void loop() {
    delay(100);  // Prevents watchdog timer resets
}